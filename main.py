...
from pg_input import TextInput
...
class App:
    def __init__(self) -> None:
        ...
        # Dynamicaly computes the height of the bar with all the sorting algorithms
        self.nav_height = self.screen.get_height() // (
            len(sorting_generators.__all__) + 1
        )
        self.font = pg.font.SysFont("Arial", 20)

        # Creates and initialize all the UI elements and containers
        self.buttons = pg.sprite.Group()
        Button(
            pg.Rect(0, 0, 100, self.nav_height),
            "Shuffle",
            self.font,
            "White",
            (222, 69, 69),
        ...
        Button(
            pg.Rect(105, 0, 100, self.nav_height),
            "Sort",
            self.font,
            "White",
            (74, 222, 69),
        ...
        self.text_inputs = pg.sprite.Group()
        TextInput(
            pg.Rect(210, 0, 100, self.nav_height),
            self.font,
            "white",
            (51, 51, 51),
            self.init_array,
        ...
        TextInput(
            pg.Rect(315, 0, 100, self.nav_height),
            self.font,
            "white",
            (51, 51, 51),
            self.set_delay,
        ...
        DropDown(
            pg.Rect(420, 0, 200, self.nav_height),
            self.font,
            sorting_generators.__all__,
            self.dropdowns,
            color="white",
            bg_color=(51, 51, 51),
        ...
        self.sort_surf = pg.Surface(
            (self.screen.get_width(), self.screen.get_height() - self.nav_height)
        )
        self.column_width = 0
        self.add_padding = True  # Takes care of the padding between the blocks
        self.array = []
    ...
    def init_array(self, length):
        ...
        if length != 0:
            ...
            self.column_width = (
                self.sort_surf.get_width() - (length if self.add_padding else 0)
            ) / length
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.MOUSEBUTTONDOWN:
                ...
                for text_input in self.text_inputs:
                    text_input.set_focus(text_input.rect.collidepoint(event.pos))
    ...
    def draw_array(self):
        ...
        for i, value in enumerate(self.array):
            ...
            line_rect.height = (
                value / len(self.array) * self.sort_surf.get_height()
            )
        ...
        self.screen.blit(
            self.sort_surf, (0, self.screen.get_height() - self.sort_surf.get_height())
        )
    ...
    def draw(self):
        ...
        pg.draw.line(
            ...
            "GREY",
            (0, self.nav_height),
            (self.screen.get_width(), self.nav_height),
        )

        self.buttons.draw(self.screen)
        self.text_inputs.draw(self.screen)
